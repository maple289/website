...
export function FileDropArea({ enabled, onFiles, children, appearance = 'default', message = 'Drop files here to upload' }: { enabled: boolean; onFiles: (files: File[]) => void; children: ReactNode; appearance?: 'default' | 'light' | 'media'; message?: string }) {
  ...
  return <div data-drag-active={active && enabled} className={`${appearance === 'light' ? 'fm-drop-area' : appearance === 'media' ? 'media-drop' : ''} relative min-h-[240px] ${active && enabled && appearance === 'default' ? 'rounded-xl ring-4 ring-blue-400' : ''}`}
    ...
    {children}
    {active && enabled && <div className={appearance === 'light' ? 'fm-drop-message pointer-events-none absolute inset-0 z-50 flex items-center justify-center p-6 text-center' : appearance === 'media' ? 'media-drop-message pointer-events-none z-50 flex items-center justify-center p-6 text-center font-semibold' : 'pointer-events-none absolute inset-0 z-50 flex items-center justify-center rounded-xl border-2 border-dashed border-blue-400 bg-blue-950/80 p-6 text-center text-xl font-semibold text-white'}><span>{message}</span></div>}
  </div>;
}
