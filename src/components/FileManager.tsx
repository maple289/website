...
const fileIcon = (entry: Entry, size = 24) => {
  ...
  if (entry.mimeType.startsWith('image/')) return <FileImage size={size} className="text-violet-500" />;
  if (entry.mimeType.startsWith('video/')) return <FileVideo size={size} className="text-rose-500" />;
  if (entry.mimeType.startsWith('audio/')) return <FileAudio size={size} className="text-amber-600" />;
  if (entry.mimeType.includes('pdf')) return <FileText size={size} className="text-red-500" />;
  if (entry.mimeType.startsWith('text/') || entry.mimeType.includes('word')) return <FileText size={size} className="text-emerald-600" />;
  if (/zip|rar|7z|tar|gzip/.test(entry.mimeType)) return <FileArchive size={size} className="text-orange-500" />;
...
function AuthenticatedThumbnail({ entry }: { entry: Entry }) {
  ...
  return url ? <img src={url} alt={entry.name} className="h-full w-full rounded-xl object-cover" /> : <FileImage size={38} className="text-violet-500" />;
}
